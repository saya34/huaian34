...
import { equipmentAttributeBonus, equipmentById, equipmentRequirements, equipmentSize, formatBonus, type EquipmentItem, type EquipmentPosition } from "./battle/progression";
import { ensureWeeklyWeaponShop, nextWeaponShopRefresh, publicWeaponRarity, weaponGridPositions, weaponPurchasePrice, weaponSellPrice } from "./battle/weaponShop";
import { useUnifiedGame } from "./core/UnifiedGameProvider";

type DragPayload = { source: "store" | "player"; uid: string };
...
function fitBuyback(items: EquipmentItem[]) {
  const kept = [...items];
  while (kept.length && !organizeEquipment(kept, CULTIVATOR_PACK_SIZE)) kept.pop();
  return kept;
}

function formatRefreshTime(date: Date) {
  return `${date.getMonth() + 1}月${date.getDate()}日 周一 00:00`;
}

export default function WeaponMerchantPanel({ onNotice }: { onNotice: (message: string) => void }) {
  const { state, setBattle } = useUnifiedGame();
  const [shelf, setShelf] = useState<"weekly" | "buyback">("weekly");
  const [selection, setSelection] = useState<Selection>(null);
  const [message, setMessage] = useState("左边是本周兵架，右边是你的法器背包。拖过去，买卖就算成了。");

  useEffect(() => {
    setBattle((current) => {
      const weaponShop = ensureWeeklyWeaponShop(current.weaponShop, current.wmPublished, current.highestUnlockedWave, current.playerLevel);
      return weaponShop === current.weaponShop ? current : { ...current, weaponShop };
    });
  }, [setBattle]);

  const battle = state.battle;
  const shopItems = shelf === "weekly" ? battle.weaponShop.stock : battle.weaponShop.buyback;
  ...
  function handlePlayerCellDrop(event: DragEvent, x: number, y: number) {
    ...
    else movePlayerItem(payload.uid, x, y);
  }

  function handleShopDrop(event: DragEvent) {
    event.preventDefault();
    const payload = readDrag(event);
    if (payload?.source === "player") sell(payload.uid);
  }
  ...
  function ItemCard({ item, source, point }: { item: EquipmentItem; source: "store" | "player"; point: EquipmentPosition }) {
    const base = equipmentById(item.equipmentId);
    const rarity = publicWeaponRarity(item);
    const size = equipmentSize(item);
    const unknown = item.identified === false;
    const price = source === "store" ? weaponPurchasePrice(item, shelf === "buyback") : weaponSellPrice(item);
    return <button
      type="button"
      draggable
      ...
      onDoubleClick={() => source === "store" ? buy(item.uid) : sell(item.uid)}
    ...
    >
      ...
      {source === "store" && <b>◉ {price.toLocaleString()}</b>}
      <i>{size.width}×{size.height}</i>
    </button>;
  }

  const nextRefresh = nextWeaponShopRefresh();
  return <div className="weapon-merchant-workspace">
    <header className="weapon-merchant-toolbar">
      <div><small>XUANFENG WEEKLY ARMS · 暗黑式占格交易</small><h3>玄锋号兵器行</h3><p>霍青翎每周一换一批兵刃；未鉴定封匣买下后才揭示稀有度与词条。</p></div>
      <div className="weapon-week-mark"><span>本周批次</span><strong>{battle.weaponShop.weekKey || "正在开箱"}</strong><small>下次刷新 · {formatRefreshTime(nextRefresh)}</small></div>
    </header>
    <div className="weapon-trade-board">
      <section className="weapon-grid-panel vendor-grid-panel" onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={handleShopDrop}>
        <header><div><small>LEFT · 商店兵架</small><h4>{shelf === "weekly" ? "本周兵器" : "近期回购物"}</h4></div><nav><button className={shelf === "weekly" ? "active" : ""} onClick={() => { setShelf("weekly"); setSelection(null); }}>周货 <b>{battle.weaponShop.stock.length}</b></button><button className={shelf === "buyback" ? "active" : ""} onClick={() => { setShelf("buyback"); setSelection(null); }}>回购 <b>{battle.weaponShop.buyback.length}</b></button></nav></header>
        <div className="weapon-grid vendor-weapon-grid" aria-label="商店十乘四武器网格">
          {Array.from({ length: 40 }, (_, index) => <i key={index} />)}
          {shopItems.map((item) => shopPositions[item.uid] && <ItemCard key={item.uid} item={item} source="store" point={shopPositions[item.uid]} />)}
          {!shopItems.length && <p className="weapon-grid-empty">{shelf === "weekly" ? "本周兵架已售罄" : "尚未向霍青翎出售兵刃"}</p>}
        </div>
        <footer><span>双击：购买</span><span>拖到右侧：指定格购买</span><span>接收右侧拖入：出售</span></footer>
      </section>
    ...
    <section className="weapon-trade-inspector">
      ...
      {selected ? (() => {
        ...
        const action = selection?.source === "store" ? () => buy(selected.uid) : () => sell(selected.uid);
        return <div className={`weapon-selected-detail ${unknown ? "unknown" : ""}`}>
          ...
          <button onClick={action} disabled={selection?.source === "player" && base.slot !== "weapon"}>{selection?.source === "store" ? selectedIsBuyback ? `赎回 · ◉${weaponPurchasePrice(selected, true).toLocaleString()}` : `购入 · ◉${weaponPurchasePrice(selected).toLocaleString()}` : base.slot === "weapon" ? `出售 · ◉${weaponSellPrice(selected).toLocaleString()}` : "仅武器可售"}</button>
        </div>;
