...
import { SPIRIT_BEASTS, beastLevelProgress, buySpiritBeast, collectSpiritBeast, feedMaterialFor, feedSpiritBeast, livestockCapacity, loveSpiritBeast, sellSpiritBeast, spiritBeastById, syncLivestock, type SpiritBeastId } from "./livestock";

type Props = { day: number; period: Period; onBack: () => void; onNotice: (message: string) => void };

const BEAST_SPRITES: Record<SpiritBeastId, { src: string; layout: "vertical" | "horizontal" }> = {
...
function BeastSprite({ speciesId, large = false }: { speciesId: SpiritBeastId; large?: boolean }) {
...
export default function LivestockPanel({ day, period, onBack, onNotice }: Props) {
  ...
  function buy(speciesId: SpiritBeastId) {
    ...
    const result = buySpiritBeast(livestock, speciesId, tick, level);
  ...
  function love(uid: string) { const result = loveSpiritBeast(livestock, uid, tick); if (!result.ok) { announce(result.message); return; } setFarm((current) => ({ ...current, livestock: result.progress })); announce(result.message); }
  function collect(uid: string) { const result = collectSpiritBeast(livestock, uid, tick); if (!result.ok) { announce(result.message); return; } setFarm((current) => ({ ...current, livestock: result.progress })); applyEffects([{ type: "add_item", item: result.reward }, { type: "add_player_exp", amount: result.reward.rarity * 2 }]); announce(`${result.message} · 已收入乾坤行囊`); }
  ...
  return <section className="spirit-farm-panel livestock-panel" aria-label="云岫灵兽苑">
    ...
    <div className="livestock-workspace">
      <aside className="livestock-list">
        ...
        {marketOpen && <div className="livestock-market">{SPIRIT_BEASTS.map((beast) => <article key={beast.id} className={level < beast.unlockLevel ? "locked" : ""}><BeastSprite speciesId={beast.id} /><div><strong>{beast.name}</strong><small>{beast.role} · {beast.unlockLevel}阶</small></div><button type="button" disabled={level < beast.unlockLevel} onClick={() => buy(beast.id)}>◉ {beast.price}</button></article>)}</div>}
      ...
      <main className="livestock-yard">{selected ? (() => { const beast = spiritBeastById(selected.speciesId)!; const productReady = selected.state === "ready"; const canLove = selected.state === "sleeping" && tick > selected.asleepAtTick && selected.lovedAtTick < selected.asleepAtTick; const remaining = selected.state === "sleeping" ? Math.max(0, selected.readyAtTick - tick) : 0; return <>
        <div className={`livestock-beast-portrait mood-${selected.mood} state-${selected.state}`}><div className="beast-aura" /><BeastSprite speciesId={selected.speciesId} large /><i>{selected.mood === "happy" ? "♥" : selected.state === "sleeping" ? "眠" : selected.state === "ready" ? "成" : "息"}</i>{selected.state === "sleeping" && <div className="beast-sleep-particles"><b>z</b><b>z</b><b>灵</b></div>}</div>
