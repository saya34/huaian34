...
export default function LivestockPanel({ day, period, onBack, onClose, onNotice }: Props) {
  ...
  function collect(uid: string) { const result = collectSpiritBeast(livestock, uid, tick); if (!result.ok) { announce(result.message); return; } setFarm((current) => ({ ...current, livestock: result.progress })); applyEffects([{ type: "add_item", item: result.reward }, { type: "add_player_exp", amount: result.reward.rarity * 2 }]); const productName=spiritBeastById(livestock.animals.find((item) => item.uid === uid)?.speciesId ?? "")?.productName??result.reward.itemId; floatBeast(uid, `+${productName} ×${result.reward.amount}`, "collect"); feedback.float({titleKey:"livestock.collectFloat",params:{name:productName,amount:result.reward.amount},icon:"收",tone:"gold",dedupeKey:`livestock:collect:${uid}:${tick}`}); announce(`${result.message} · 已收入乾坤行囊`); }
  ...
  return <section className={`spirit-farm-panel livestock-panel ${readyTotal ? "has-ready-produce" : ""}`} aria-label="云岫灵兽苑">
    ...
    <div className="livestock-workspace">
      ...
      <main className={`livestock-yard selected-state-${selected?.state ?? "empty"}`}>{selected ? (() => { const beast = spiritBeastById(selected.speciesId)!; const productReady = selected.state === "ready"; const canLove = selected.state === "sleeping" && tick > selected.asleepAtTick && selected.loveCount < 2 && selected.lovedAtTick !== tick; const remaining = selected.state === "sleeping" ? Math.max(0, selected.readyAtTick - tick) : 0; const objective = productReady ? livestockUi.readyObjective : selected.state === "sleeping" ? livestockUi.sleepingObjective : livestockUi.idleObjective; const value = productReady ? livestockUi.readyValue : selected.state === "sleeping" ? gatheringCopy(livestockUi.sleepingValue, { count: remaining }) : livestockUi.idleValue; return <>
        ...
        <GatheringFocusHud theme="livestock" kicker={livestockUi.kicker} objective={objective} value={value} hint={livestockUi.hint} icon={productReady ? "收" : selected.state === "sleeping" ? "抚" : "饲"} attention={productReady || selected.sick} meter={productReady ? 100 : selected.state === "sleeping" ? Math.max(6, 100 - remaining / Math.max(1, beast.productionTicks) * 100) : 0} stats={[{ label: livestockUi.stats[0], value: livestock.animals.length }, { label: livestockUi.stats[1], value: readyTotal }, { label: livestockUi.stats[2], value: `${livestock.animals.length}/${livestockCapacity(level, livestock.shelterLevel)}` }]} />
        ...
        <div className={`livestock-beast-portrait mood-${selected.mood} state-${selected.state}`} onClick={() => { const copy = selected.state === "ready" ? "灵息圆满 · 可以唤醒收取" : selected.state === "sleeping" ? `吐纳中 · 尚余 ${remaining} 时辰` : "它抬头望向你，等待照料"; floatBeast(selected.uid, copy, "info"); const profile=beastLevelProgress(selected.experience); feedback.inspect({titleKey:"livestock.animalTitle",icon:"兽",imageSrc:beast.productArt,details:[{labelKey:"relationship.nameLabel",value:beast.name,emphasis:true},{labelKey:"livestock.levelLabel",value:`Lv.${profile.level}`},{labelKey:"livestock.stateLabel",value:feedbackText(selected.state==="ready"?"livestock.stateReady":selected.state==="sleeping"?"livestock.stateSleeping":"livestock.stateIdle")},{labelKey:"livestock.moodLabel",value:feedbackText(selected.mood==="happy"?"livestock.moodHappy":"livestock.moodNormal")},{labelKey:"livestock.healthLabel",value:feedbackText(selected.sick?"livestock.healthSick":"livestock.healthGood")},{labelKey:"livestock.feedLabel",value:beast.feedMaterialName},{labelKey:"livestock.productLabel",value:beast.productName},{labelKey:"livestock.readyLabel",value:selected.state==="sleeping"?remaining:feedbackText(selected.state==="ready"?"livestock.stateReady":"livestock.stateIdle")}],dedupeKey:`livestock:inspect:${selected.uid}:${tick}`}); }}><div className="beast-aura" /><BeastSprite speciesId={selected.speciesId} large /><i>{selected.mood === "happy" ? "♥" : selected.state === "sleeping" ? "眠" : selected.state === "ready" ? "成" : "息"}</i>{selected.state === "sleeping" && <div className="beast-sleep-particles"><b>z</b><b>z</b><b>灵</b></div>}{beastFx?.uid === selected.uid && <span className={`beast-floating-info ${beastFx.kind}`}>{beastFx.text}</span>}</div>
        ...
        <div className="livestock-actions"><button type="button" className={selected.state === "idle" && !selected.sick ? "primary-gather-action" : ""} disabled={selected.state !== "idle" || selected.sick} onClick={() => feed(selected.uid)}><b>饲</b><span><strong>常食 ×{beast.feedQuantity}</strong><small>{beast.feedMaterialName} · 持有 {state.shared.items[feedMaterialFor(selected.speciesId).id]?.amount ?? 0}</small></span></button><button type="button" disabled={selected.state !== "idle" || selected.sick} onClick={() => feed(selected.uid, true)}><b>喜</b><span><strong>喜食加速成长</strong><small>{beast.favoriteMaterialName} · 持有 {state.shared.items[favoriteFeedFor(selected.speciesId).id]?.amount ?? 0}</small></span></button><button type="button" className={canLove ? "primary-gather-action" : ""} disabled={!canLove} onClick={() => love(selected.uid)}><b>{interactionTool === "pet" ? "抚" : interactionTool === "brush" ? "梳" : "曲"}</b><span><strong>亲和照料 {selected.loveCount}/2</strong><small>{selected.state !== "sleeping" ? "需先投喂" : canLove ? `当前需要${selected.interactionNeed === "pet" ? "抚摸" : selected.interactionNeed === "brush" ? "梳理" : "奏乐"}` : "下一时辰开放"}</small></span></button>{selected.sick ? <button type="button" className="cure primary-gather-action" onClick={() => cure(selected.uid)}><b>药</b><span><strong>请宁绾秋诊治</strong><small>消耗灵石 48</small></span></button> : <button type="button" className={productReady ? "ready primary-gather-action" : ""} disabled={!productReady} onClick={() => collect(selected.uid)}><b>醒</b><span><strong>唤醒并收取</strong><small>{beast.productName}收入行囊</small></span></button>}</div>
      </>; })() : <><GatheringFocusHud theme="livestock" kicker={livestockUi.kicker} objective={livestockUi.emptyObjective} value={livestockUi.emptyValue} hint={livestockUi.hint} icon="迎" meter={0} stats={[{ label: livestockUi.stats[0], value: 0 }, { label: livestockUi.stats[1], value: 0 }, { label: livestockUi.stats[2], value: `0/${livestockCapacity(level, livestock.shelterLevel)}` }]} /><div className="livestock-yard-empty"><span>苑</span><h3>灵兽苑静候有缘</h3><p>迎入灵兽后，可在此投喂、睡眠抚灵并在生产结束后唤醒收取。</p></div></>}</main>
    ...
    <footer className="farm-message" aria-live="polite"><span>兽</span><p>{message}</p><b>生产时序 {tick + 1} · 只随游戏时间推进</b></footer>
  </section>;
}
