...
export default function LivestockPanel({ day, period, onBack, onClose, onNotice }: Props) {
  ...
  return <section className={`spirit-farm-panel livestock-panel ${readyTotal ? "has-ready-produce" : ""}`} role="dialog" aria-modal="true" aria-label="云岫灵兽苑">
    ...
    <div className="livestock-workspace">
      ...
      </>; })() : <><GatheringFocusHud theme="livestock" kicker={livestockUi.kicker} objective={livestockUi.emptyObjective} value={livestockUi.emptyValue} hint={livestockUi.hint} icon="迎" meter={0} stats={[{ label: livestockUi.stats[0], value: 0 }, { label: livestockUi.stats[1], value: 0 }, { label: livestockUi.stats[2], value: `0/${livestockCapacity(level, livestock.shelterLevel)}` }]} /><div className="livestock-yard-empty"><span>苑</span><h3>灵兽苑静候有缘</h3><button type="button" onClick={onClose ?? onBack}><i>寻</i><b>{livestockUi.emptyAction}</b><small>{livestockUi.emptyActionHint}</small></button></div></>}</main>
