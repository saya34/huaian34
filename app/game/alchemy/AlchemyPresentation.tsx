...
export function AlchemyResultOverlay({ open, item, mutation, firstObtain, onCollect, onReset }: { open: boolean; item: GameItem; mutation: MutationId; firstObtain: boolean; onCollect: () => void; onReset: () => void }) {
  if (!open) return null;
  const damaged=mutation==="burnt"||mutation==="flawed";
  const exalted=!damaged&&firstObtain&&(mutation==="perfect"||item.rarity>=5);
  const presentation=damaged?"damaged":exalted?"exalted":"routine";
  const title=damaged?["炉火有失","药 性 受 损"]:exalted?["灵韵凝华","上 品 成 丹"]:["一炉既成","灵 丹 出 炉"];
  return <div className={`result-overlay outcome-${presentation} mutation-${mutation}`} role="dialog" aria-modal="true" aria-label="炼丹结果"><div className="result-rays" aria-hidden="true" /><div className="result-card"><div className="card-art product-card-art"><span className="result-item-aura" aria-hidden="true" /><img className="result-item-icon" src={item.image} alt={mutationDisplayName(item, mutation)} /><span className="legendary-tag">{mutation === "normal" ? `${item.quality}·${item.category}` : `${MUTATIONS[mutation].prefix}·异变词缀`}</span></div><div className="card-copy"><span className="first-acquired">◈ {firstObtain?"首次炼得":"本炉成品"} · 待收入成品库</span><h2 className={`mutation-name mutation-${mutation}`}>{mutationDisplayName(item, mutation)}</h2><p>{MUTATIONS[mutation].note}。{item.effect}。五行属 <b>{item.element}</b>，实际估值 <b>{getMutationValue(item, mutation).toLocaleString()}</b> 灵石。</p>{mutation !== "normal" && <div className={`mutation-banner mutation-${mutation}`}><span>{damaged?"火候偏差":"炼丹异变"}</span><strong>{MUTATIONS[mutation].prefix}</strong><small>价值倍率 ×{MUTATIONS[mutation].valueMultiplier}</small></div>}<div className="result-value"><span>灵韵稀有</span><strong>{"◆".repeat(item.rarity)}</strong></div><div className="card-actions"><button onClick={onCollect}>收入成品库</button><button onClick={onReset}>收下并再炼</button></div></div></div><div className="result-title"><span>{title[0]}</span><strong>{title[1]}</strong></div></div>;
}
