...
export default function FishingEncounter({ fishId, rarity, config, onFinish }: Props) {
  ...
  const [castSettled, setCastSettled] = useState(false);
  const finishedRef = useRef(false);
  const phaseRef = useRef<FishingEncounterPhase>(phase);
  phaseRef.current = phase;

  useEffect(() => {
    const timer = window.setTimeout(() => setCastSettled(true), 880);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setBiteMissed(true);
      window.setTimeout(() => onFinish({ success: false, score: 0, attempts: 1, failureReason: "missed-bite" }), 620);
  ...
  const chooseGiveTake = (actual: GiveTakeCommand) => {
    ...
    if (result.failed) {
      finishedRef.current = true;
      window.setTimeout(() => onFinish({ success: false, score: 0, attempts: giveTakeIndex + 1, failureReason: "escaped" }), 650);
      return;
    }
  ...
  return <div className={`fishing-encounter phase-${phase} rarity-${rarity} ${stepResult ? `step-${stepResult}` : ""} ${biteMissed ? "bite-missed" : ""} ${castSettled ? "cast-settled" : "cast-entering"}`} role="application" tabIndex={0} onKeyDown={onKeyDown} aria-label={phaseLabel}>
    ...
    <main className="encounter-stage">
      <div className="encounter-cast-line" aria-hidden="true"><i /><b /><u /></div>
      <div className={`encounter-float ${phase === "bite" ? "biting" : ""}`}><i /><b /><span /></div>
      <div className={`encounter-fish-shadow rarity-${rarity}`}><i /><b /><u /></div>
      {phase === "waiting" && <section className="encounter-message waiting-message"><small>{phaseLabel}</small><h3>{reelUi.waitingTitle}</h3><p>{reelUi.waitingBody}</p><div className="waiting-dots"><i /><i /><i /></div><b>{reelUi.waitingHint}</b><button type="button" className="mobile-thumb-action encounter-wait-action" disabled><strong>{reelUi.waitingAction}</strong><small>{reelUi.waitingActionHint}</small></button></section>}
      {phase === "bite" && <section className="encounter-message bite-message"><small>{phaseLabel}</small><h3>{biteMissed ? reelUi.biteMiss : reelUi.biteTitle}</h3><p>{reelUi.biteBody}</p><button type="button" className="mobile-thumb-action" onClick={judgeBite}><i /><strong>{reelUi.biteAction}</strong><small>{reelUi.biteKeyboard}</small></button></section>}
      {phase === "give-take" && <section className="encounter-message give-take-message"><small>{phaseLabel}</small><h3>{reelUi.giveTakeTitle}</h3><p>{reelUi.giveTakeBody}</p><div className={`give-take-prompt command-${expected}`}><i /><strong>{expected === "reel" ? reelUi.giveTakePromptReel : reelUi.giveTakePromptRelease}</strong><span>{stepResult === "correct" ? reelUi.giveTakeCorrect : stepResult === "wrong" ? reelUi.giveTakeWrong : `${reelUi.giveTakeBalance} ${balance >= 0 ? "+" : ""}${balance}`}</span></div><div className="give-take-actions"><button type="button" className={expected === "reel" ? "mobile-thumb-action" : "mobile-thumb-secondary"} onClick={() => chooseGiveTake("reel")}><b>{reelUi.giveTakeReel}</b><span>{reelUi.giveTakeReelHint}</span></button><button type="button" className={expected === "release" ? "mobile-thumb-action" : "mobile-thumb-secondary"} onClick={() => chooseGiveTake("release")}><b>{reelUi.giveTakeRelease}</b><span>{reelUi.giveTakeReleaseHint}</span></button></div><div className="give-take-progress">{encounter.giveTakePattern.map((_, index) => <i key={index} className={index < giveTakeIndex ? "done" : index === giveTakeIndex ? "active" : ""} />)}</div></section>}
    </main>
  </div>;
}
