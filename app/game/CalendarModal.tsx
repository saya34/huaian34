...
export default function CalendarModal({ state, events, onClose }: { state: GameState; events: EventDefinition[]; onClose: () => void }) {
  ...
  return <div className="calendar-backdrop" role="presentation" onMouseDown={onClose}><section className="calendar-shell" role="dialog" aria-modal="true" aria-label="云和历" onMouseDown={(event) => event.stopPropagation()}>
    ...
    <footer><div className="calendar-selected-date"><span>{selectedDay}</span><div><small>{getCalendarDate(selectedAbsolute).weekdayName}</small><strong>{CALENDAR_MONTHS[view.month - 1]}{getCalendarDate(selectedAbsolute).dayName}</strong></div></div><div className="calendar-event-list">{selectedActivities.map((activity) => <article role="button" tabIndex={0} onClick={()=>feedback.inspect({titleKey:"calendar.activityTitle",bodyKey:"calendar.activityBody",params:{name:activity.name,subtitle:activity.subtitle},icon:activity.icon,details:[{labelKey:"calendar.dateLabel",value:`第 ${selectedAbsolute} 日`},{labelKey:"calendar.ruleLabel",value:activity.scheduleLabel},{labelKey:"world.locationLabel",value:activity.sceneId}],dedupeKey:`calendar-activity:${activity.id}:${selectedAbsolute}`})} key={activity.id} className="calendar-activity-item"><i className={`calendar-doodle ${activity.id === "tavern-gambling" ? "gambling" : "market"}`}><b>{activity.icon}</b></i><div><small>周期活动 · {activity.scheduleLabel}</small><strong>{activity.name}</strong><p>{activity.subtitle} · 前往对应场景参加</p></div></article>)}{selectedEvents.map((event) => <article role="button" tabIndex={0} onClick={()=>feedback.inspect({titleKey:"calendar.eventTitle",bodyKey:"calendar.eventBody",params:{name:event.title,subtitle:event.subtitle},icon:DOODLE_LABELS[event.calendarEvent?.doodle??"story"].slice(0,1),details:[{labelKey:"calendar.dateLabel",value:`第 ${selectedAbsolute} 日`},{labelKey:"relationship.nameLabel",value:event.characterId},{labelKey:"world.locationLabel",value:event.sceneId},{labelKey:"calendar.ruleLabel",value:DOODLE_LABELS[event.calendarEvent?.doodle??"story"]}],dedupeKey:`calendar-event:${event.id}:${selectedAbsolute}`})} key={event.id}><i className={`calendar-doodle ${event.calendarEvent?.doodle ?? "story"}`}><b>{DOODLE_LABELS[event.calendarEvent?.doodle ?? "story"].slice(0, 1)}</b></i><div><small>{DOODLE_LABELS[event.calendarEvent?.doodle ?? "story"]} · {event.calendarEvent?.mode === "weekly" ? `每${CALENDAR_WEEKDAYS[(event.calendarEvent.weekday ?? 1) - 1]}` : event.calendarEvent?.mode === "interval" ? `每隔 ${event.calendarEvent.everyDays} 日` : "固定日期"}</small><strong>{event.title}</strong><p>{event.subtitle}</p></div></article>)}{!selectedEvents.length && !selectedActivities.length && <p className="calendar-empty">此日无预定事项，宜随心而行。</p>}</div></footer>
  </section></div>;
}
