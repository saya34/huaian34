...
export type MiningMapId = "yunzhou" | "village" | "wilds" | "canglan" | "chixia";
...
export const MINING_LOCATIONS:MiningLocation[]=[
  {id:"yunzhou-mine",name:"玄铁常明矿窟",subtitle:"常驻矿洞 · 深层迷宫可持续下探",kind:"resident",mapId:"wilds",x:82,y:25,pool:[{materialName:"黑曜火铁",weight:42},{materialName:"星陨铁",weight:30},{materialName:"雷纹紫晶",weight:20},{materialName:"太初玉髓",weight:8}]},
  {id:"yunzhou-vein",name:"野外游光地宫",subtitle:"随机矿脉 · 探索完成后消失",kind:"random",mapId:"wilds",pool:[{materialName:"星陨铁",weight:40},{materialName:"黑曜火铁",weight:32},{materialName:"金乌翎石",weight:20},{materialName:"太初玉髓",weight:8}]},
...
export function rollMiningMaterial(location:MiningLocation,seed:string,depthBias=0,maxRarity=5):GameItem{const full=miningPool(location),eligible=full.filter(entry=>entry.material.rarity<=maxRarity),lowest=Math.min(...full.map(entry=>entry.material.rarity)),available=eligible.length?eligible:full.filter(entry=>entry.material.rarity===lowest);const entries=available.map((entry,index)=>({...entry,adjusted:entry.probability*(1+depthBias*index*.24)}));const total=entries.reduce((sum,entry)=>sum+entry.adjusted,0);let roll=hash(seed)*total;for(const entry of entries){roll-=entry.adjusted;if(roll<=0)return entry.material;}return entries.at(-1)!.material;}
...
function rollGroundReward(location:MiningLocation,tile:MineTile,serial:number,maxRarity=5):MiningReward|null{
  const soil=SOIL_DEFINITIONS[tile.kind as MineSoilType],depthRatio=Math.min(1,tile.depth/9),image="/assets/activities/treasure-atlas.webp";
  // Every cleared soil block rolls once. Rarer strata and deeper rooms improve both discovery odds and reward quality.
  const discoveryChance=Math.min(.94,.28+soil.rarity*.11+depthRatio*.2);
  if(hash(`${tile.rewardSeed}:found:${serial}`)>discoveryChance)return null;
  const roll=hash(`${tile.rewardSeed}:drop:${serial}`);
  if(roll<.08+depthRatio*.05){const keyId:MineKeyId=depthRatio>.72?"star":depthRatio>.38?"jade":"bronze",key=KEY_DEFINITIONS[keyId];return{kind:"key",keyId,name:key.name,amount:1,rarity:key.rarity,image};}
  if(roll<.25)return{kind:"currency",name:"地脉灵石",amount:12+soil.rarity*5+Math.floor(depthRatio*48),rarity:1,image:"/assets/activities/spirit-stone-atlas.webp"};
  if(roll<.88){const material=rollMiningMaterial(location,`${tile.rewardSeed}:ore`,depthRatio+soil.rarity*.16,maxRarity);return{kind:"item",itemId:material.id,itemType:"material",name:material.name,amount:1+(tile.kind==="ancient"?1:0),rarity:Math.max(1,Math.min(7,material.rarity)) as 1|2|3|4|5|6|7,image:material.image};}
  return{kind:"item",itemId:"buried-ancient-coin",itemType:"treasure",name:"地宫古钱",amount:1,rarity:2,image:"/assets/items/item-32.webp"};
}

export function strikeMineTile(progress:MiningProgress,input:{location:MiningLocation;spotId?:string;tileId:string}):{progress:MiningProgress;ok:boolean;message:string;cleared?:boolean;reward?:MiningReward;damage?:number;durabilityCost?:number}{const maze=activeMiningMaze(progress,input.location,input.spotId);if(!maze)return{progress,ok:false,message:"这处地宫已经消散"};const tile=maze.tiles.find(item=>item.id===input.tileId);if(!tile||tile.state!=="revealed")return{progress,ok:false,message:"必须先挖通相邻土块才能抵达此处"};if(tile.kind==="wall")return{progress,ok:false,message:"镇脉黑墙不可挖掘，只能寻找绕行路线"};if(tile.kind==="chest"||tile.kind==="deep-chest")return{progress,ok:false,message:"这是宝箱，请使用开启操作"};if(tile.kind==="entrance")return{progress,ok:false,message:"这里是本层入口"};const soil=SOIL_DEFINITIONS[tile.kind];if(progress.pickaxeDurability<soil.durabilityCost)return{progress,ok:false,message:"玄铁灵镐耐久不足，请先修复"};const damage=powerFor(progress.pickaxeLevel),hp=Math.max(0,tile.hp-damage),cleared=hp<=0;let tiles=maze.tiles.map(item=>item.id===tile.id?{...item,hp,state:cleared?"dug" as const:item.state}:item);if(cleared)tiles=revealAround(tiles,{...tile,hp:0,state:"dug"});const nextMaze={...maze,tiles,deepestOpened:Math.max(maze.deepestOpened,tile.depth)};let next=replaceMaze({...progress,pickaxeDurability:progress.pickaxeDurability-soil.durabilityCost,strikeSerial:progress.strikeSerial+1,totalMined:progress.totalMined+(cleared?1:0)},input.location,nextMaze,input.spotId);const reward=cleared?rollGroundReward(input.location,tile,progress.strikeSerial,progress.pickaxeTier):null;if(reward?.kind==="key"&&reward.keyId)next={...next,keys:{...next.keys,[reward.keyId]:next.keys[reward.keyId]+reward.amount}};if(reward?.kind==="item"&&reward.itemId)next={...next,records:{...next.records,[reward.itemId]:(next.records[reward.itemId]??0)+reward.amount}};return{progress:next,ok:true,message:cleared?reward?`挖通${soil.name} · 灵光迸现，发现${reward.name} ×${reward.amount}`:`挖通${soil.name} · 土屑散落，本次未发现可用之物`:`凿击${soil.name} · ${hp}/${tile.maxHp}`,cleared,reward:reward??undefined,damage,durabilityCost:soil.durabilityCost};}
