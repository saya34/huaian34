...
function rollGroundReward(location:MiningLocation,tile:MineTile,serial:number):MiningReward|null{
  ...
  if(roll<.88){const material=rollMiningMaterial(location,`${tile.rewardSeed}:ore`,depthRatio+soil.rarity*.16);return{kind:"item",itemId:material.id,itemType:"material",name:material.name,amount:1+(tile.kind==="ancient"?1:0),rarity:Math.max(1,Math.min(7,material.rarity)) as 1|2|3|4|5|6|7,image:material.image};}
