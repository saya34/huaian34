...
import { PICKAXE_PRICE, mineNode, miningLocationById, miningMaterialByName, miningNodes, miningPool, resetMiningDay, type MiningLocationId } from "./mining";
...
export default function MiningModal({ locationId, randomSpotId, day, period, onClose, onNotice }: Props) {
  ...
  return <div className="mining-backdrop" role="presentation" onMouseDown={onClose}><section className="mining-window" role="dialog" aria-modal="true" aria-label={`${location.name}挖矿`} onMouseDown={(event) => event.stopPropagation()}>
    ...
    <div className="mining-layout">
      ...
      <main className="mining-face"><div className="vein-status"><span>可开采矿点</span><strong>{available} / {nodes.length}</strong><i><u style={{ width: `${nodes.length ? Math.max(0, available / nodes.length * 100) : 0}%` }} /></i></div>
        {nodes.length ? <><div className="ore-node-field"><div className="ore-cave-glow" />{nodes.map((node, index) => { const material = miningMaterialByName(node.materialName); const ready = location.kind === "resident" ? node.readyAtTick <= tick : node.minedAtTick < 0; const remaining = Math.max(0, node.readyAtTick - tick); return <button type="button" key={node.id} disabled={!ready} className={`${ready ? "ready" : "recovering"} ${struckNode === node.id ? "struck" : ""}`} style={{ "--node-delay": `${index * .12}s` } as React.CSSProperties} onClick={() => strike(node.id)}><span className="ore-node-rock"><img src={material.image} alt=""/><i /></span><strong>{ready ? material.name : `复原 ${remaining}`}</strong><small>{ready ? "点击挥镐" : "游戏时辰"}</small>{struckNode === node.id && <em>✦ ×{result?.amount ?? 1}</em>}</button>; })}</div><p>矿点就是开采对象；工具消耗、产物入包与节点复原会在同一次结算中完成。</p></> : <div className="mine-exhausted"><span>尽</span><h3>{location.kind === "resident" ? "矿点正在复原" : "游光矿脉已消散"}</h3><p>{location.kind === "resident" ? "推进游戏内时辰后，常明矿窟会逐块重新凝结。" : "该光点已从山河图移除，新的矿脉会在后续游戏日出现。"}</p></div>}
      </main>
