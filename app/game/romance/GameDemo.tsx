...
const STORAGE_KEY = "cloud-romance-event-demo-v1";
...
export default function GameDemo() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) setGame({ ...INITIAL_STATE, ...JSON.parse(saved), activeEvent: null });
    } catch {
      // A broken local demo save should never prevent the game from opening.
    }
    setHydrated(true);
  }, []);
  ...
  useEffect(() => {
    let active = true;
    Promise.all([
      fetch("/api/em/gifts?status=published", { cache: "no-store" }).then((response) => response.ok ? response.json() : Promise.reject()),
      fetch("/api/em/dialogues?status=published", { cache: "no-store" }).then((response) => response.ok ? response.json() : Promise.reject()),
      fetch("/api/em/global-keys?status=published", { cache: "no-store" }).then((response) => response.ok ? response.json() : Promise.reject()),
  ...
  useEffect(()=>{
    let active=true;
    fetch("/api/em/messages?status=published",{cache:"no-store"}).then((response)=>response.ok?response.json():Promise.reject()).then((data:{messages?:Array<{definition:CharacterMessageDefinition}>})=>{if(active)setManagedMessages((data.messages??[]).map((item)=>item.definition))}).catch(()=>{/* Built-in letters remain available. */});
    return()=>{active=false};
  },[]);
  ...
  useEffect(() => {
    ...
    fetch("/api/em/content?status=published", { cache: "no-store" })
      .then((response) => response.ok ? response.json() : Promise.reject(new Error("内容库暂不可用")))
  ...
  useEffect(() => {
    ...
    fetch("/api/em/events?status=published", { cache: "no-store" })
      .then((response) => response.ok ? response.json() : Promise.reject(new Error("事件库暂不可用")))
  ...
  useEffect(() => {
    if (!hydrated) return;
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...game, activeEvent: null }));
  }, [game, hydrated]);
