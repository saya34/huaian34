...
import { chapterOneOutcomeRewardLabels, resolveChapterOneOutcome, type ChapterOneOutcome } from "../chapter-one/service";
...
export function QuestOfferDialogue({ definition, onClose }: { definition: QuestDefinition; onClose: () => void }) {
  ...
  function accept() {
    ...
    if (definition.startsProject === "medicine-shortage" && state.romance.medicineShortage.status === "offered") setRomance((current) => ({ ...current, medicineShortage: { status: "active", acceptedDay: current.day, deadlineDay: current.day + 2, battleVictories: 0, farmHarvestsAtAccept: state.farm.totalHarvests } }));
...
export default function QuestPanel({ onClose, onNavigate }: { onClose: () => void; onNavigate: QuestNavigate }) {
  ...
  function claim(definition: QuestDefinition) {
    ...
    const outcome = definition.id === "main-chapter-one-return" ? resolveChapterOneOutcome(state) : null;
    applyEffects(questRewardEffects(definition, state));
    setQuests((current) => {
