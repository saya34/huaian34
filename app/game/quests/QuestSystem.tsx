...
export default function QuestPanel({ onClose, onNavigate }: { onClose: () => void; onNavigate: QuestNavigate }) {
  ...
  return <div className="quest-panel-backdrop" onMouseDown={onClose}><section className="quest-panel quest-journal-v2" role="dialog" aria-modal="true" aria-label={questText("panelTitle")} onMouseDown={(event) => event.stopPropagation()}>
    ...
    {receipt && <div className="quest-reward-backdrop"><section className={`quest-reward-modal ${receipt.id === "main-chapter-one-return" ? "chapter-finale" : ""}`} role="dialog" aria-modal="true"><img src={ART} alt=""/><span>{receipt.id === "main-chapter-one-return" ? "槐" : "成"}</span><small>{receipt.completion?.eyebrow ?? questText("rewardTitle")}</small><h3>{receipt.completion?.title ?? receipt.name}</h3>{receipt.completion?.body && <p className="quest-completion-story">{receipt.completion.body}</p>}<p>{questText("rewardObtained")}</p><div>{receipt.rewards.map((reward, index) => <b key={`${reward.type}-${index}`}>{reward.label} ×{reward.amount}</b>)}</div>{receipt.completion?.nextHook && <blockquote>{receipt.completion.nextHook}</blockquote>}<button type="button" className="quest-primary" onClick={() => setReceipt(null)}>{questText("continue")}</button></section></div>}
  </section></div>;
}
