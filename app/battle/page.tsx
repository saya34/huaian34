...
export default async function BattlePage({ searchParams }: { searchParams: Promise<{ wave?: string; embedded?: string; ready?: string; framed?: string }> }) {
  ...
  return <GameProviders><div className={embedded ? "battle-embedded-page" : "battle-standalone-page"}><div className="rotate-hint"><b>{mobileGuidance.title}</b><span>{mobileGuidance.subtitle}</span><p>{mobileGuidance.controls}</p></div><MowingGame initialWaveId={waveId} embedded={embedded} autoStart={autoStart} /></div></GameProviders>;
}
